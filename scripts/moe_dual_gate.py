...
"""
Script description:
Script for training Mixture of Experts models with dual gating networks for emotion
...
Usage:
$ python moe_dual_gate.py <dataset> <config_path> [--weight_triggers] [--train_bert]
"""
...
class MoEForEmotionAndTriggerClassification(nn.Module):
    ...
    This architecture uses dual gating networks to dynamically weight expert modules
    ...
    def forward(
        ...
        expert_weights_emotion = torch.softmax(
            self.gating_network_emotion(pooled_embeddings), dim=-1
        )

        expert_weights_trigger = torch.softmax(
            self.gating_network_trigger(pooled_embeddings), dim=-1
        )

        combined_output_emotion = self._compute_expert_output(
            embeddings, expert_weights_emotion
        )
        combined_output_trigger = self._compute_expert_output(
            embeddings, expert_weights_trigger
        )

        combined_output_emotion = self.dropout(combined_output_emotion)
        combined_output_trigger = self.dropout(combined_output_trigger)

        emotion_logits = self.emotion_classifier(combined_output_emotion)
        trigger_logits = self.trigger_classifier(combined_output_trigger).squeeze(-1)
...
def _run_single_experiment(
    ...
    # file_name = (
    #     f"moe_model_{train_bert}_train_bert_{gate_type}_dual_gate_"
...
def run_experiments(
    ...
    """
    Runs experiments utilizing Mixture of Experts model with dual gating mechanism.
